...
describe('Check that containers are ready', () => {
  ...
  describe('Supplier1 containers', () => {
    test('Supplier1 messenger GET /health-check returns 200', async () => {
      const res = await request(supplier1MessengerURL).get('/api/v1/health-check');
      expect(res.statusCode).toEqual(200);
    });

    test('Supplier1 radish-api REST server GET /health-check returns 200', async () => {
      const res = await request(supplier1ApiURL).get('/api/v1/health-check');
      expect(res.statusCode).toEqual(200);
    });
  });

  describe('Supplier2 containers', () => {
    test('Supplier2 messenger GET /health-check returns 200', async () => {
      const res = await request(supplier2MessengerURL).get('/api/v1/health-check');
      expect(res.statusCode).toEqual(200);
    });

    test('Supplier2 radish-api REST server GET /health-check returns 200', async () => {
      const res = await request(supplier2ApiURL).get('/api/v1/health-check');
      expect(res.statusCode).toEqual(200);
    });
  });
